...
func (v *VM) Dispatch() {
	...
	case OP_SET_LOCAL:
		slot := v.GetOperand().(*ObjInteger).Value
	...
	case OP_FOR_LOOP:
		...
		v.ForLoop(rgInit,bytes)
	case OP_EQUAL:
		rval := v.Pop()
		lval := v.Pop()

		if lval.Type() == VAL_INTEGER && rval.Type() == VAL_INTEGER {
			if lval.(*ObjInteger).Value == rval.(*ObjInteger).Value {
				v.Push(&ObjBool{true})
			} else {
				v.Push(&ObjBool{false})
			}
		}
